...
int TestMain::exec(const bool testAudio)
{
    ...
    if (modernOpenGLTest == 0)
    {
        ...
        {
            modernOpenGLFps = readTestResult(17);
    ...
    if (normalOpenGLTest == 0)
    {
        ...
        {
            normalOpenGLFps = readTestResult(9);
    ...
    if (safeOpenGLTest == 0)
    {
        ...
        {
            safeOpenGLFps = readTestResult(10);
...
int TestMain::readTestResult(int wantedTestId)
{
    int fallback = 0; // default if reading failed or no value.
    std::ifstream file(pathJoin(settings.localDataDir, "test.log"));
    int testId, result;
    file >> testId >> result; // >> walks over newlines.
    if (!file || wantedTestId != testId)
        result = fallback;

    log->log("result of video test %d: %d", wantedTestId, result);
    return result;
}
