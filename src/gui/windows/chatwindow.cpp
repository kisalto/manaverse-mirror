...
void ChatWindow::initTradeFilter()
{
    std::ifstream tradeFile(
        pathJoin(settings.serverConfigDir, "tradefilter.txt"));
    std::string line;
    line.reserve(32); // just simple phrases
    while (std::getline(tradeFile, line))
    {
        if (!line.empty())
            mTradeFilter.push_back(line);
    }
}
...
void ChatWindow::loadCustomList()
{
    std::ifstream listFile(
        pathJoin(settings.serverConfigDir, "customwords.txt"));
    std::string line;
    line.reserve(32);
    while (std::getline(listFile, line))
    {
        if (!line.empty())
            mCustomWords.push_back(line);
    }
}
