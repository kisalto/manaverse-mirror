...
void ShopWindow::loadList()
{
    ...
    const std::string shopListName(
        pathJoin(settings.serverConfigDir, "shoplist.txt"));

    std::ifstream shopFile(shopListName);
    if (!shopFile)
    {
    ...
    std::string line;
    while (std::getline(shopFile, line))
    {
        std::stringstream iss(line);
        int itemId, buyQty, buyPrice, sellQty, sellPrice;
        if (iss >> itemId >> buyQty >> buyPrice >> sellQty >> sellPrice)
        {
            if (itemId && buyQty && buyPrice)
