...
MiniStatusWindow::MiniStatusWindow() :
    ...
    mMaxX(0)
{
    StatusWindow::updateHPBar(mHpBar, false);

    if (statusWindow != nullptr)
        statusWindow->updateMPBar(mMpBar, false);

    const bool job = serverConfig.getValueBool("showJob", true);

    StatusWindow::updateXPBar(mXpBar, true);
...
void MiniStatusWindow::setIcon(const int index, AnimatedSprite *const sprite)
{
    ...
    if (AnimatedSprite *const oldSprite = mIcons[index])
    {
        mIconsTotalWidth -= (mIconSpacing + oldSprite->getWidth());
        delete oldSprite;
    }
    mIcons[index] = sprite;
    mIconsTotalWidth += (mIconSpacing + sprite->getWidth());
}
...
{
    int startX = mMaxX + mIconPadding;
    for (const AnimatedSprite *const icon : mIcons)
    {
        if (icon != nullptr)
        {
            icon->draw(graphics, startX, mIconPadding);
            startX += mIconSpacing + icon->getWidth();
        }
    }
}

void MiniStatusWindow::drawIconsCentered(Graphics *const graphics,
                                         int centerX,
                                         int topY) const
{
    int startX = centerX
               // the front spacing is redundant.
               - (mIconsTotalWidth - mIconSpacing) / 2
               - mapTileSize / 2; // spritedef.cpp centering
    int startY = topY + mIconPadding;
    for (const AnimatedSprite *const icon : mIcons)
    {
        if (icon != nullptr)
        {
            const int width = icon->getWidth();
            // spritedef.cpp automatically centers images at bottom
            // middle of tile. Partially undo this
            const int x = startX + width / 2;
            const int y = startY;
            icon->draw(graphics, x, y);
            startX += mIconSpacing + width;
        }
    }
}
...
void MiniStatusWindow::updateStatus()
{
    if (statusWindow != nullptr)
    {
        statusWindow->updateStatusBar(mStatusBar, true);
    }
...
void MiniStatusWindow::mouseMoved(MouseEvent &event)
{
    ...
    else if (event.getSource() == mJobBar)
    {
        ...
        const std::string level = strprintf(_("Job level: %d"), jobLevel);
