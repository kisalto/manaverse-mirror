        ...
        case UpdateDownloadStatus::UPDATE_RESOURCES:
            if (mDownloadComplete)
            {
                if (CAST_SIZE(mUpdateIndex) < mUpdateFiles.size())
                {
                    ...
                    std::string path(pathJoin(mUpdatesDir, mCurrentFile));
                    std::ifstream temp(path);

                    mValidateXml = false;
                    if (!temp || !validateFile(path, mCurrentChecksum))
                    {
                        download();
                    }
                    else
