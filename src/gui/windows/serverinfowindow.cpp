...
ServerInfoWindow::ServerInfoWindow(ServerInfo &serverInfo) :
    ...
    Window(_("Game info"), Modal_false, nullptr, "serverinfo.xml"),
    LinkHandler(),
...
void ServerInfoWindow::showServerInfo()
{
    ...
    if (type == ServerFreeType::Free)
    {
        ...
        mBrowserBox->addRow(_("##BGame with free license."),
            false);
    }
    else if (type == ServerFreeType::NonFree)
    {
        // TRANSLATORS: server info comment
        mBrowserBox->addRow(_("##BGame with non-free license."),
            false);
    }
    ...
    {
        // TRANSLATORS: server info comment
        mBrowserBox->addRow(_("##BGame with unknown license."),
            false);
    }
