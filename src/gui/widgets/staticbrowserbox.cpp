                              ...
                              const bool atTop)
{
    ...
    if (mUseLinksAndUserColors)
    {
        ...
        const int rowCount = static_cast<int>(mTextRows.size());
        ...
        // [@@link|caption@@]          https://example.com
        //  ^startIdx       ^endIdx    ^startIdx          ^endIdx
        //    ^linkIdx                 ^linkIdx
        //        ^delimIdx
        const char* starts[] = {"http://", "https://", "@@"};
        size_t startIdx, endIdx;
        while ((startIdx = findFirstOf(tmp, starts)) != string::npos)
        {
            const bool isURL = (tmp[startIdx] != '@');
            if (isURL)
            {
                endIdx = startIdx + 1;
                while (endIdx < tmp.size() && !isspace(tmp[endIdx]))
                    ++endIdx;
                if (endIdx == string::npos) // can this even happen?
                    endIdx = tmp.size();
                const size_t linkIdx = startIdx;
                bLink.link = tmp.substr(linkIdx, endIdx - linkIdx);
                bLink.caption = string();
            }
            else
            {
                const size_t linkIdx = startIdx + 2; // skip @@
                size_t delimIdx = tmp.find('|', linkIdx);
                endIdx = tmp.find("@@", delimIdx);
                if (delimIdx == string::npos ||
                    endIdx == string::npos)
                {
                    tmp.erase(0, 2); // safe
                    continue; // maybe another http[s]:// ?
                }
                bLink.link = tmp.substr(linkIdx, delimIdx - linkIdx);
                const size_t capIdx = delimIdx + 1;
                bLink.caption = tmp.substr(capIdx, endIdx - capIdx);
                endIdx += 2; // skip last @@
            }

            bLink.y1 = rowCount * font->getHeight();
            ...
            newRow.append(tmp.substr(0, startIdx));

            string tmp2 = newRow;
            // remove text colour modifiers
            for (size_t modIdx; (modIdx = tmp2.find("##")) != string::npos; )
                tmp2.erase(modIdx, 3); // this is safe
            ...
            tmp.erase(0, endIdx);
            if (!tmp.empty())
                newRow.append("##>"); // apply previous colour
        }
    ...
    for (size_t modIdx; (modIdx = plain.find("##")) != string::npos; )
        plain.erase(modIdx, 3); // this is safe
            ...
            if (start == 0 || mUseLinksAndUserColors)
            {
                ...
                if (row.find("##", start) == start && row.size() > start + 2)
                {
                    ...
                    const Color col[2] =
                    {
                        ...
                        getThemeCharColor(CAST_S8(c | 0x80), valid)
                    };
