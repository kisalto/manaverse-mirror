...
void SetupItem::save()
{
    ...
    // Inefficient (at the time of this writing, 416 lookups on a map),
    // but not noticeable even on a slow core2duo.
    // ::action could set flags, but would need more listeners on
    // e.g. TextField and would be prone to forgetting stuff. KISS.
    if (cfg->getStringValue(mKeyName) != mValue)
    {
        // don't record potentially personal info like proxy url and such.
        logger->log("SETUP: set value for key %s", mKeyName.c_str());
