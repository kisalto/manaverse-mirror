                ...
                !isProtected)
            {
                ...
                if (cnt > 1)
                {
                    mBrowserBox->addRow("/addtrade 'INVINDEX' 1",
                        // TRANSLATORS: popup menu item
                        // TRANSLATORS: add item amount to trade
                        strprintf(_("Add to trade %d"), 1).c_str());
                    ...
                    if (cnt > 10)
                    {
                        mBrowserBox->addRow("/addtrade 'INVINDEX' 10",
                            // TRANSLATORS: popup menu item
                            // TRANSLATORS: add item amount to trade
                            strprintf(_("Add to trade %d"), 10).c_str());
                    }
            ...
            if (InventoryWindow::isStorageActive())
            {
                ...
                if (cnt > 1)
                {
                    mBrowserBox->addRow("/invtostorage 'INVINDEX' 1",
                        // TRANSLATORS: popup menu item
                        // TRANSLATORS: add item amount to storage
                        strprintf(_("Store %d"), 1).c_str());
        ...
        case InventoryType::Storage:
            ...
            if (cnt > 1)
            {
                mBrowserBox->addRow("/storagetoinv 'INVINDEX' 1",
                    // TRANSLATORS: popup menu item
                    // TRANSLATORS: get item amount from storage
                    strprintf(_("Retrieve %d"), 1).c_str());
                ...
                if (cnt > 10)
                {
                    mBrowserBox->addRow("/storagetoinv 'INVINDEX' 10",
                        // TRANSLATORS: popup menu item
                        // TRANSLATORS: get item amount from storage
                        strprintf(_("Retrieve %d"), 10).c_str());
                }
