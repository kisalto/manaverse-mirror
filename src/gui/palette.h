    ...
    protected:
        ...
        struct ColorElem final
        {
            ColorElem() :
                type(0),
                color(0),
                colorChannelSettings(0),
                committedColorChannelSettings(0),
                description(),
                ch(0),
            ...
            /*
             * Each color element has 3 tuneables: the color channel
             * properties (R,G,B,A) values, the animation type (gradient)
             * and the animation delay.
             * Each of these three has their "committed" version, which is
             * cached for quick rollback in the colors section of SETUP.
             */

            // FIXME: Actually UserColorIdT
            int type;
            // The color used for drawing, either static or generated
            // from channel settings. (RGB color model)
            Color color;
            // the color used for setting animation color properties
            Color colorChannelSettings;
            Color committedColorChannelSettings;
            // (translated) text shown to the user in menus
            std::string description;
            signed char ch;
            // colour animation type
            GradientTypeT grad;
            GradientTypeT committedGrad;
            // colour animation step counter
            int gradientIndex;
            // user-selected animation/gradient transition delay
            int delay;
            int committedDelay;
