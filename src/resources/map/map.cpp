...
void Map::addExtraLayer() restrict2
{
    ...
    if (mapFile)
    {
        ...
        line.reserve(50); // 40 was about the longest I could find locally.
        ...
        while (std::getline(mapFile, line))
        {
            ...
            if (iss >> x >> y >> type)
            {
                ...
                if (type == MapItemType::PORTAL)
                    updatePortalTile(comment, type, x, y, false);
                else if (type == MapItemType::HOME)
                    updatePortalTile(comment, type, x, y, true);
                else
                    addPortalTile(comment, type, x, y);
            }
        }
    }
    BLOCK_END("Map::addExtraLayer")
