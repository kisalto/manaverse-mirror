...
static void deleteSpriteList(std::list<EmoteSprite*> &sprites)
{
    while (!sprites.empty())
    {
...
void EmoteDB::unload()
{
    ...
    for (auto &info : mEmoteInfos)
    {
        if (info.second != nullptr)
        {
            deleteSpriteList(info.second->sprites);
            delete info.second;
        }
    }
    mEmoteInfos.clear();
    deleteSpriteList(mUnknown.sprites);
    mLoaded = false;
}
