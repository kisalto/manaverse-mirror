...
/*
 * Turns &[0-9]+; into special characters given
 * by their decimal code between & and ;
 */
...
/**
 * Finds the first occurance of any of the given strings in haystack.
 *
 * @param haystack the string to search
 * @param needles array of C-strings to search for
 * @return index of first occurence, or ::npos if not found.
 */
template<size_t N>
size_t findFirstOf(const std::string &haystack, const char* (&needles)[N])
{
    // inefficient, for now
    size_t pos, min_index = std::string::npos;
    for (size_t i = 0; i < N; ++i)
        if ((pos = haystack.find(needles[i])) != std::string::npos)
            if (pos < min_index)
                min_index = pos;
    return min_index;
}
