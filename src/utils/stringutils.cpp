...
{
    std::string::size_type ampPos = text.find('&');
    // potentially avoid copying (CoW & RVO)
    ...
    std::string::size_type sectionStartPos = 0; // after last ;
    while (ampPos != std::string::npos)
    {
        std::string::size_type numPos = ampPos + 1;
        if (numPos == std::string::npos)
            break;

        const char *start = text.c_str() + numPos;
        char *end;
        // there used to be an atoi() call here, hence base 10.
        unsigned long number = strtoul(start, &end, 10);

        std::string::size_type digitCount = end - start;
        std::string::size_type semiPos = numPos + digitCount;
        if (semiPos == std::string::npos)
            break;
        else if (text[semiPos] == ';' && 1 <= digitCount && number < 256)
        {
            // +1 so it can be overwritten, ampPos + 1 = numPos.
            // (append has no method taking a char)
            processed.append(text, sectionStartPos, numPos - sectionStartPos);
            processed.back() = static_cast<char>(number);
            sectionStartPos = semiPos + 1;
            ampPos = text.find('&', sectionStartPos);
        }
        else
            ampPos = text.find('&', numPos);
    }
    processed.append(text, sectionStartPos, std::string::npos);
    return processed;
}
