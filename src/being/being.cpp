                       ...
                       const int level) restrict2
{
    ...
    if (chatWindow != nullptr && mShowBattleEvents)
    {
        if (this == localPlayer)
        {
            ...
            {
                ChatWindow::battleChatLog(strprintf("%s : Missed you",
        ...
        else if (attacker == localPlayer)
        {
            ...
            {
                ChatWindow::battleChatLog(strprintf("%s : You missed %s",
                    attacker->mName.c_str(),
                    mName.c_str()),
                    ChatMsgType::BY_PLAYER,
                    IgnoreRecord_false,
                               ...
                               const int y) const restrict2
{
        ...
        mAction != BeingAction::DEAD)
    {
        const int barW = 2 * 50;
        const int barH = 4;
        int barX = x - barW / 2 + mapTileSize / 2 + mInfo->getHpBarOffsetX();
        ...
        if (miniStatusWindow != nullptr)
            miniStatusWindow->drawIconsCentered(
                graphics,
                x + mapTileSize / 2 + mInfo->getHpBarOffsetX(),
                barY + barH + 1);
    }
}
