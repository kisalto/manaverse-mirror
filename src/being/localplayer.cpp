...
LocalPlayer::LocalPlayer(const BeingId id,
    ...
    mServerAttack(fromBool(serverConfig.getBoolValue("serverSideAttack"),
                  Keep)),
    ...
    mFreezed(false)
{
    ...
    config.addListener("serverSideAttack", this);
...
void LocalPlayer::optionChanged(const std::string &value)
{
    ...
    else if (value == "serverSideAttack")
    {
        mServerAttack
            = fromBool(config.getBoolValue("serverSideAttack"), Keep);
    }
...
bool LocalPlayer::pickUpItems(int pickUpType)
{
    ...
    FloorItem *item = actorManager->findItemWithin(x, y, x, y);
    if (item != nullptr)
        status = pickUp(item);
    ...
    switch (pickUpType)
    {
        case 1:
            ...
            item = actorManager->findItemWithin(x, y, x, y);
            break;
        case 2:
            switch (mDirection)
            {
                case BeingDirection::UP:
                    item = actorManager->findItemWithin(x - 1, y - 1,
                                                        x + 1, y);
                    break;
                case BeingDirection::DOWN:
                    item = actorManager->findItemWithin(x - 1, y,
                                                        x + 1, y + 1);
                    break;
                ...
                case BeingDirection::RIGHT:
                    item = actorManager->findItemWithin(x,     y - 1,
                                                        x + 1, y + 1);
                    break;
            }
            break;
        case 3:
            item = actorManager->findItemWithin(x - 1, y - 1, x + 1, y + 1);
            break;
        case 4:
            item = actorManager->findNearestItem(x, y, 4);
            break;
        case 5:
            item = actorManager->findNearestItem(x, y, 8);
            break;
        case 6:
            item = actorManager->findNearestItem(x, y, 90);
            break;
        default:
            break;
    }
    ...
    if (layer != nullptr)
    {
        ...
        if (mShowServerPos)
        {
                ...
                mapItem->getType() == MapItemType::EMPTY)
            {
                if (mX != x || mY != y)
                {
                    // 1/3x1/3 tile black block visible for a few moments.
