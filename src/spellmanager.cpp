...
void SpellManager::loadDefaultSpells()
{
    CommandsDB::load();

    for (const auto &pair : CommandsDB::getAll())
        addSpell(pair.second, true);

    CommandsDB::unload();
}

bool SpellManager::addSpell(TextCommand *const spell, bool clobber)
{
    if (spell == nullptr)
        return false;
    ...
    else if (clobber)
    {
        ...
        delete mSpellsVector.at(id);
        mSpellsVector.at(id) = spell;
    }
    return false;
}
...
void SpellManager::load()
{
    ...
    if (!haveSavedSpell) // mSpells* storage is filled with blanks.
        loadDefaultSpells();
}
