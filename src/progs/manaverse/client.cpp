...
void Client::gameInit()
{
    ...
    mCurrentServer.type = ServerInfo::parseType(settings.options.serverType);
...
void Client::stateConnectServer1()
{
    if (mOldState == State::CHOOSE_SERVER)
    {
        ...
        ConfigManager::initServerConfig(mCurrentServer.hostname,
                                        mCurrentServer.type);
        // TODO: This is a sledgehammer. It would be better to
        // just not add the per-server options to SETUP until after
        // the server is known and its default config loaded.
        if (setupWindow != nullptr) // unnecessary check
