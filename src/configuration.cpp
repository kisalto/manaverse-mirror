...
// Add latest removals to the top of this list: For performance reasons
// a match on the first key is used to trigger mass removal of the rest.
const char *const unusedKeys[] =
{
    "showChatHistory",
    "serverAttack",
...
bool ConfigurationObject::deleteKey(const std::string &key)
{
    return (mOptions.erase(key) > 0); // true if key was removed.
}
...
void ConfigurationObject::deleteList(ConfigurationList &list)
{
    for (ConfigurationObject *obj : list)
        delete obj;
    list.clear();
}
...
{
    for (auto &entry : mContainerOptions)
        deleteList(entry.second);
...
{
    for (auto &entry : mDefaultsData)
        delete entry.second;
    mDefaultsData.clear();
}

void Configuration::setDefault(const std::string &key,
                               VariableData *data)
{
    auto pair = mDefaultsData.insert({key, data});
    if (!pair.second) // there was already such a key
    {
...
void Configuration::removeOldKeys()
{
    ...
    {
        for (int index = 0; index != NUM_ELEMENTS(unusedKeys); ++index)
        {
            if (deleteKey(unusedKeys[index]))
                logger->log("removed unused key: %s", unusedKeys[index]);
        }
    }
}
